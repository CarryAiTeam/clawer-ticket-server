...
export function createTicketMcpServer({ getApplication }: TicketMcpServerDependencies): McpServer {
  ...
  server.registerTool(
    "ticket_my_open_tasks",
    {
      ...
      inputSchema: { profile: z.string().min(1), limit: z.number().int().min(1).max(1_000).default(1_000), includeDetails: z.boolean().default(true) },
  ...
  server.registerTool(
    "ticket_get",
    {
      ...
      inputSchema: { profile: z.string().min(1), ticket: ticketSchema },
      annotations: { readOnlyHint: true, openWorldHint: true },
    },
  ...
  server.registerTool(
    "ticket_export",
    {
      ...
      inputSchema: { profile: z.string().min(1), ticket: ticketSchema, mode: z.enum(["plan", "write"]).default("plan"), media: z.enum(["metadata", "download"]).default("download") },
      annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
    },
    /** 处理单张工单的导出计划或写入请求。 */
  ...
  server.registerTool(
    "ticket_export_my_open_tasks",
    {
      ...
      inputSchema: { profile: z.string().min(1), limit: z.number().int().min(1).max(1_000).default(1_000), mode: z.enum(["plan", "write"]).default("plan"), media: z.enum(["metadata", "download"]).default("download"), statuses: z.array(z.string().min(1).max(128)).max(20).optional() },
      annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
    },
